...
from generators import (
    ...
    enumerate_normal, enumerate_m_grouped_contiguous, enumerate_m_grouped_masked, enumerate_m_grouped_masked_transpose, enumerate_k_grouped_contiguous, enumerate_m_grouped_masked_transpose_n_group, enumerate_normal_transpose,generate_normal_2d1d_transpose,
...
def test_gemm_2d1d_transpose() -> None:
    ...
    for kernel_type, m, n, k, major_a, major_b, accumulate, out_dtype in enumerate_normal_transpose(torch.float8_e4m3fn):
        ...
        func_name = f'fp8_gemm_tn_transpose'
        getattr(deep_gemm, func_name)(a, b, d, c=c, disable_ue8m0_cast=disable_ue8m0_cast, recipe=recipe)
        diff = calc_diff(d, ref_d)
        assert diff < 0.001, (f'{m=}, {n=}, {k=}, {kernel_opt}, {major_opt=}, {accumulate=}, {out_dtype=}, '
                                  f'{diff:.5f}')
