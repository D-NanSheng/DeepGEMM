...
sm90_fp8_gemm_2d1d_transpose_n_group_sbo_impl(float* sfa, int* grouped_layout,
                        uint32_t shape_m, uint32_t shape_n, uint32_t shape_k,
                        const __grid_constant__ cute::TmaDescriptor tensor_map_a,
                        const __grid_constant__ cute::TmaDescriptor tensor_map_b,
                        const __grid_constant__ cute::TmaDescriptor tensor_map_d,
                        const __grid_constant__ cute::TmaDescriptor tensor_map_sfb,
                        const uint32_t* recv_signal, uint32_t* send_signal) {
    ...
    const uint32_t num_block_tokens = ceil_div(shape_n, BLOCK_N);
    ...
    if (warp_idx >= kNumMathThreads / 32) {
        ...
        if (warp_idx == kNumMathThreads / 32 + 2 and cute::elect_one_sync()) {
            ...
            while (scheduler.get_next_block(m_block_idx, n_block_idx)) {
                ...
                for (uint32_t k_block_idx = 0; k_block_idx < num_total_k_blocks; advance_pipeline(k_block_idx)) {
                    ...
                    // Wait the k_block_idx-th block of n_block_idx token block is ready
                    // while(ld_acquire_global_64(signal+n_block_idx) <= k_block_idx){}; // 获取signal[第n个token块]的第k_block_idx部分数据量是否到达。<=k_block_idx,说明还未到达。
                    // Issue TMA A
        ...
        // TMA checks , I put TMA checks here for send_signal with last block/wave
        ...
        while (scheduler.get_next_block(m_block_idx, n_block_idx)) {
            ...
            // I don't put atomic add operation here, because the performance is not good. I guess it is because blocking the stsm.
        ...
        //     asm volatile("cp.async.bulk.wait_group 0;\n" ::: "memory");
        // }
        // cutlass::arch::NamedBarrier::sync(kNumMathThreads, 1); // cutlass::arch::NamedBarrier(kNumMathThreads).sync();
        // if (threadIdx.x == 0) {
        //     atomic_add_release_global_64(send_signal + (scheduler.current_group_idx-1) * num_block_tokens + n_block_idx, 1); // note that current_group_idx is the previous group
        // }
