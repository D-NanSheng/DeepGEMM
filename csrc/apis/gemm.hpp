...
static void fp8_gemm_tn_transpose(const std::pair<torch::Tensor, torch::Tensor>& a,
                        const std::pair<torch::Tensor, torch::Tensor>& b,
                        const torch::Tensor& d,
                        const std::optional<torch::Tensor>& c,
                        std::optional<std::tuple<int, int, int>> recipe,
    ...
    const auto& [m , k ] = get_shape<2>(a.first);
    const auto& [n , k_] = get_shape<2>(b.first);
    const auto& [n_, m_] = get_shape<2>(d); // 输出是转置的
    DG_HOST_ASSERT(m == m_ and n == n_ and k == k_);
    DG_HOST_ASSERT(a.first.scalar_type() == torch::kFloat8_e4m3fn);
    DG_HOST_ASSERT(b.first.scalar_type() == torch::kFloat8_e4m3fn);
    DG_HOST_ASSERT(d.scalar_type() == torch::kBFloat16 or d.scalar_type() == torch::kFloat);
    ...
    if (not recipe.has_value())
        recipe = get_default_recipe(a.second.scalar_type(), b.second.scalar_type());
    DG_HOST_ASSERT(recipe.value() == std::make_tuple(1, 1, 128) or recipe.value() == std::make_tuple(1, 128, 128));
    const auto& sfa = layout::transform_sf_into_required_layout(a.second, m, k, recipe.value(), std::nullopt,  false, disable_ue8m0_cast); // <128, 128>
    const auto& sfb = layout::transform_sf_into_required_layout(b.second, n, k, recipe.value(), std::nullopt, true, disable_ue8m0_cast); // scale<1,128>

    // Dispatch into different implements
    const auto& arch_major = device_runtime->get_arch_major();
    if (arch_major == 9 and sfa.scalar_type() == torch::kFloat) {
        if (std::get<1>(recipe.value()) == 1) {
            DG_HOST_UNREACHABLE("fp8_gemm_tn_transpose unsupported for 1D1D");
            // sm90_fp8_gemm_1d1d(a.first, sfa, b.first, sfb, c, d, m, n, k, major_a, major_b, compiled_dims);
        } else {
            const auto& major_sfa = get_major_type_ab(sfa);
            sm90_fp8_gemm_2d1d_transpose(a.first, sfa, b.first, sfb, c, d, m, n, k, major_a, major_b, major_sfa, compiled_dims);
        }
    } else if (arch_major == 10 and sfa.scalar_type() == torch::kInt) {
        DG_HOST_UNREACHABLE("Unsupported architecture or scaling factor types");
        // sm100_fp8_gemm_1d1d(a.first, sfa, b.first, sfb, c, d, m, n, k, major_a, major_b, compiled_dims);
...
// 这里的输入是a和b已经转置过的
static void m_grouped_fp8_gemm_tn_masked(const std::pair<torch::Tensor, torch::Tensor>& a,
                                         const std::pair<torch::Tensor, torch::Tensor>& b,
                                         const torch::Tensor& d,
                                         const torch::Tensor& masked_m,
                                         const int& expected_m,
    ...
    if (arch_major == 9 and sfa.scalar_type() == torch::kFloat) {
        const auto& major_sfa = get_major_type_ab(sfa);
        sm90_m_grouped_fp8_gemm_masked_2d1d(a.first, sfa, b.first, sfb, d, masked_m,
                                            num_groups, m, n, k, expected_m, major_a, major_b, major_sfa, compiled_dims);
    } else {
        DG_HOST_UNREACHABLE("Unsupported architecture or scaling factor types");
    }
}

// 这里的输入是a和b已经转置过的，在计算过程中会将输出矩阵转置后再输出
static void m_grouped_fp8_gemm_tn_transpose_masked(const std::pair<torch::Tensor, torch::Tensor>& a,
                                         const std::pair<torch::Tensor, torch::Tensor>& b,
                                         const torch::Tensor& d,
                                         const torch::Tensor& masked_m,
                                         const int& expected_m,
    ...
    if (arch_major == 9 and sfa.scalar_type() == torch::kFloat) {
        const auto& major_sfa = get_major_type_ab(sfa);
        sm90_m_grouped_fp8_gemm_masked_2d1d_transpose(a.first, sfa, b.first, sfb, d, masked_m,
                                            num_groups, m, n, k, expected_m, major_a, major_b, major_sfa, compiled_dims);
    } else {
        DG_HOST_UNREACHABLE("Unsupported architecture or scaling factor types");
    }
...
static void m_grouped_fp8_gemm_tn_n_group_masked(const std::pair<torch::Tensor, torch::Tensor>& a,
                                         const std::pair<torch::Tensor, torch::Tensor>& b,
                                         const torch::Tensor& d,
    ...
    if (arch_major == 9 and sfa.scalar_type() == torch::kFloat) {
        const auto& major_sfa = get_major_type_ab(sfa);
        sm90_m_grouped_fp8_gemm_masked_2d1d_n_group(a.first, sfa, b.first, sfb, d, masked_n,
...
static std::optional<std::pair<int, int>> m_grouped_fp8_gemm_nt_sbo_masked(const std::pair<torch::Tensor, torch::Tensor>& a,
                                         const std::pair<torch::Tensor, torch::Tensor>& b,
                                         const torch::Tensor& d,
                                         const torch::Tensor& masked_m,
                                         const int& expected_m,
    ...
    if (arch_major == 9 and sfa.scalar_type() == torch::kFloat) {
        const auto& major_sfb = get_major_type_ab(sfb);
        result = sm90_m_grouped_fp8_gemm_masked_1d2d_sbo(a.first, sfa, b.first, sfb, d, masked_m,
                                            num_groups, m, n, k, expected_m, major_a, major_b, major_sfb, compiled_dims, recv_signal, send_signal);
    } else if (arch_major == 10 and sfa.scalar_type() == torch::kInt) {
        sm100_m_grouped_fp8_gemm_masked_1d1d(a.first, sfa, b.first, sfb, d, masked_m,
                                             num_groups, m, n, k, expected_m, major_a, major_b, compiled_dims);
    } else {
...
static void register_apis(pybind11::module_& m) {
    ...
    m.def("fp8_gemm_tn_transpose", &fp8_gemm_tn_transpose,
          py::arg("a"), py::arg("b"), py::arg("d"),
          py::arg("c") = std::nullopt, py::arg("recipe") = std::nullopt,
          py::arg("compiled_dims") = "nk",
          py::arg("disable_ue8m0_cast") = false);
    ...
    m.def("m_grouped_fp8_gemm_tn_masked", &m_grouped_fp8_gemm_tn_masked,
          py::arg("a"), py::arg("b"), py::arg("d"), py::arg("masked_m"),
          py::arg("expected_m"), py::arg("recipe") = std::nullopt,
          py::arg("compiled_dims") = "nk", py::arg("disable_ue8m0_cast") = false);
    m.def("m_grouped_fp8_gemm_tn_transpose_masked", &m_grouped_fp8_gemm_tn_transpose_masked,
          py::arg("a"), py::arg("b"), py::arg("d"), py::arg("masked_m"),
          py::arg("expected_m"), py::arg("recipe") = std::nullopt,
          py::arg("compiled_dims") = "nk", py::arg("disable_ue8m0_cast") = false);
    m.def("m_grouped_fp8_gemm_tn_n_group_masked", &m_grouped_fp8_gemm_tn_n_group_masked,
    ...
    m.def("m_grouped_fp8_gemm_tn_transpose_n_group_masked", &m_grouped_fp8_gemm_tn_transpose_n_group_masked,
          ...
          py::arg("expected_n"), py::arg("recipe") = std::nullopt,
          py::arg("compiled_dims") = "nk", py::arg("disable_ue8m0_cast") = false);
    m.def("m_grouped_fp8_gemm_nt_sbo_masked", &m_grouped_fp8_gemm_nt_sbo_masked,
          py::arg("a"), py::arg("b"), py::arg("d"), py::arg("masked_m"),
          py::arg("expected_m"), py::arg("recipe") = std::nullopt,
          py::arg("compiled_dims") = "nk", py::arg("disable_ue8m0_cast") = false,
